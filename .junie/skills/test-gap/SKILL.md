...
1. Establish current coverage:
   `uv run pytest tests/unit/ -q --cov=extract_version --cov-report=term-missing`
   (scope with `--cov=extract_version.<subpkg>` when a path is given).
2. Spawn the **`testing-expert`** agent with the coverage output and the target path. Ask it
   ...
   (blast-radius criteria + Extract Version module examples per tier):
