...
This project defines **no gates beyond a green suite plus `/pr-review`** - there is no
coverage floor, no benchmark, no staging deploy, and no standing security review. Do not
invent any. The gates are exactly what `.github/workflows/python-app.yml` runs, and they
are cheap enough to run in full at every M5:
...
2. `release-saga --local-install` - build the wheel and install it. CI tests the
...
Note that the kit's agent prompts reference `uv run pytest --cov`; this repo has neither
...
The default above stands for this project: ratified divergences are recorded in the milestone
`status.md` under `## Notes & decisions`. There is no ADR requirement (the repo ships no
`docs/adr/` yet) and no design authority beyond `AskUserQuestion`, so step 3 needs no
adjustment. Two project-specific additions when the ruling touches the published API - the
keyword-only signatures of `validate_version`, `extract_version`, `sort_versions`,
`available_versions`, `get_last_version`, or the `""` failure sentinel:

- Update the `## Architecture` section of the repo-root `CLAUDE.md`, which is currently the
  only durable design record for this codebase.
- Add the user-visible consequence to `RELEASE_NOTES.json` under
  `releases.<VERSION>.release_notes`, since `extract-version` is published to PyPI and that
  file is the changelog consumers actually see.
...
`release-saga --local-install`, and `python test/test_extract_version.py` (see
