...
- The cross-cutting rules in `@docs/dev/python_coding_standard.md` - full annotations, ruff,
  ...
  The repo-root `CLAUDE.md` adds conventions the shipped standard does not cover, and where
  the two disagree `CLAUDE.md` describes what this repo actually does - name these sections
  for the agent: **`## Commands`** (tests are stdlib `unittest` run as
  `python test/test_extract_version.py`, not `pytest`; lint is `flake8` with
  `--select=E9,F63,F7,F82`, not `ruff`), **`## Architecture`** (`test/test_extract_version.py`
  is the single test file and uses real fixture directories under
  `test/test_data/versions/{cellar,pycharm}/` rather than mocking the filesystem - add fixture
  dirs there; `src/examples/*.py` mirror the `README.md` examples and must be updated with any
  API change), and **`## Notes`** (`src/` layout, so imports are
  `extract_version.version_info` with no `src.` prefix; Python `>=3.11`; CI builds and
  installs the wheel and tests the *installed* package). There is no `AGENTS.md`, no
  logging/redaction utility, and no separate security policy in this repo.
...
| Condition                                                | Skill                                                                                                                                |
|----------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------|
| Always                                                   | `/test-gap src/extract_version/<subtask-module>/` - report coverage delta; if < 85 % delegate missing tests to `testing-expert`. |
| `pyproject.toml` or any `requirements*.txt` changed      | `/dep-audit`                                                                                                                         |
| Any file under an API route, auth, or middleware path changed, or anywhere handling credentials/tokens | `/secret-scan src/extract_version/<changed-module>/` |
...
- **Task complete** → this is the **one** place the whole suite runs: execute
  `uv run pytest tests/unit/ -q --cov=extract_version` once (summary line only), then run
